...
int main() {
	 ...
	 * Otherwise we will have conflicts since each pin will try to
	 * set its own period and conflict with the others */
	pinA.Enable();
	pinB.Enable();
	...
	/* Release the pins so they are disabled and so that we can also
	 * reset their periods
	 * Note if you just call Disable() then you cant change the period
	 * as both channels will be treated as in use */
	pinA.Release();
	pinB.Release();
